...
for (const variant of ['medium', 'hard', 'impossible']) {
  ...
  t('honors accumulated anti-loop bans even when a banned slide would win', async () => {
    const engine = await createWasmEngine(bytes, mkOpts({ maxDepth: 4, nodeBudget: 200000, seed: 7 }));
    const s = newGame();
    // The grid-slide-win position, but X's winning slide (1,2) → (2,2) is on
    // the accumulated ban list (along with an earlier undone slide) after O
    // kept undoing. Repeating either is illegal.
    s.board[idx(3, 1)] = X; s.board[idx(3, 2)] = X; s.board[idx(3, 3)] = X; s.board[idx(0, 0)] = X;
    s.board[idx(0, 2)] = O; s.board[idx(0, 3)] = O; s.board[idx(4, 0)] = O; s.board[idx(4, 4)] = O;
    s.placed[X] = 4; s.placed[O] = 4;
    s.reserve[X] = 0; s.reserve[O] = 0;
    s.center = { r: 1, c: 2 };
    s.lastSlideFrom = { r: 2, c: 2 }; // O's latest undo slid (2,2) → (1,2)
    s.bannedSlideTos = [{ r: 1, c: 1 }, { r: 2, c: 2 }]; // accumulated list
    const move = await engine.chooseMove(s);
    assert.ok(isLegal(s, move), `illegal move ${JSON.stringify(move)}`);
    const landsOnBanned =
      move.type === 'grid' &&
      s.bannedSlideTos.some(
        (p) => s.center.r + move.dr === p.r && s.center.c + move.dc === p.c
      );
